(function(Scratch) {
    ...
    class RayCasting {
        ...
        _renderFrame() {
            ...
            // Возвращаем исходное затемнение
        ...
        _renderLight(light) {
            ...
            const gradient = this.shadowCtx.createRadialGradient(
                ...
                lightPos.x, lightPos.y, light.size
            );
            ...
            for (let i = 0; i < rayCount; i++) {
                ...
                const rayEnd = this._castRay(lightPos, angle, light.size * 1.5);
        ...
        _castRay(start, angle, maxDist) {
            ...
            for (const target of targets) {
                ...
                try {
                    ...
                    let spriteCanvas;
                    if (drawable.skin._texture) {
                        spriteCanvas = drawable.skin._texture.canvas;
                    } else if (drawable.skin._canvas) {
                        spriteCanvas = drawable.skin._canvas;
                    } else {
                        continue;
                    }
                    ...
                    // Получаем параметры спрайта в координатах Scratch
                    const scratchX = target.x;
                    const scratchY = target.y;
                    const scale = target.size / 100;
                    const rotation = (90 - target.direction) * Math.PI / 180;

                    // Преобразуем координаты начала луча в координаты Scratch
                    const canvasScale = Math.min(this.width / 480, this.height / 360);
                    const rayStartX = (start.x - this.width/2) / canvasScale;
                    const rayStartY = -(start.y - this.height/2) / canvasScale;

                    // Направление луча с инвертированной Y-координатой
                    ...
                    for (let i = 0; i < maxSteps; i++) {
                        const worldX = rayStartX + rayDirX * (i * step);
                        const worldY = rayStartY + rayDirY * (i * step);

                        const relX = worldX - scratchX;
                        const relY = worldY - scratchY;

                        const rotX = relX * Math.cos(-rotation) - relY * Math.sin(-rotation);
                        const rotY = relX * Math.sin(-rotation) + relY * Math.cos(-rotation);

                        const textureX = Math.floor((rotX / scale + spriteCanvas.width/2));
                        const textureY = Math.floor((-rotY / scale + spriteCanvas.height/2));
                        ...
                        if (textureX >= 0 && textureX < spriteCanvas.width && 
                            ...
                            if (alpha > 10) {
                                const hitX = this.width/2 + worldX * canvasScale;
                                const hitY = this.height/2 - worldY * canvasScale;
            ...
            return closestPoint || {
                x: start.x + Math.cos(angle) * maxDist,
                y: start.y + Math.sin(angle) * maxDist
            };
        }
        ...
        _rayPixelIntersection(start, angle, imageData, sprite) {
            ...
            for (let dist = 0; dist < maxSteps; dist += step) {
                ...
                if (textureX >= 0 && textureX < sprite.width && 
                    ...
                    if (alpha > 10) { // Уменьшенный порог для лучшего определения
        ...
        _getAccurateShape(canvas, bounds, scale, spriteCoords, rotation) {
            try {
                ...
                // Преобразуем сглаженные радиусы в точки
        ...
        _findEdgeRadius(imageData, centerX, centerY, angle, width, height) {
            ...
            for (let r = 0; r <= maxRadius; r += step) {
                ...
                if (Math.abs(alpha - lastAlpha) > 50) { // Порог различия
                    return r;
                }
                lastAlpha = alpha;
            }
        ...
        _getSpriteVertices(target) {
            ...
            // Применяем поворо�� и смещение к каждой вершине
            return vertices.map(v => {
        ...
        _convertToCanvasCoords(x, y) {
            // Получаем масштаб для преобразования координат Scratch в координаты canvas
